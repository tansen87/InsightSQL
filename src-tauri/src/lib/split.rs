...
pub async fn split_lines(csv_options: CsvOptions<&str>, size: u32, output_path: &str) -> Result<()> {
...
pub async fn split(
  ...
  let file_stem = Path::new(path.as_str()).file_stem().unwrap().to_str().unwrap();
  ...
  match mode.as_str() {
    "rows" => {
      match csv_options.indexed().map_err(|e| e.to_string())? {
        ...
        None => sequential_split_rows(csv_options, size, &output_path).await.map_err(|e| e.to_string())?,
      };
