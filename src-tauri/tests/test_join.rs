...
async fn test_left_outer_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "left", false).await?;
...
async fn test_right_outer_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "right", false).await?;
  let context = std::fs::read_to_string(&output_path)?;
  let result = context.trim().split('\n').collect::<Vec<_>>();
...
async fn test_full_outer_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "full", false).await?;
  let context = std::fs::read_to_string(&output_path)?;
  let result = context.trim().split('\n').collect::<Vec<_>>();
  let expected = vec![
    "idx,name,idx,age",
...
async fn test_cross_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "cross", false).await?;
  let context = std::fs::read_to_string(&output_path)?;
  let result = context.trim().split('\n').collect::<Vec<_>>();
  let expected = vec![
    "idx,name,idx,age",
...
async fn test_inner_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "inner", false).await?;
  let context = std::fs::read_to_string(&output_path)?;
  let result = context.trim().split('\n').collect::<Vec<_>>();
...
async fn test_left_semi_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "left_semi", false).await?;
  let context = std::fs::read_to_string(&output_path)?;
  let result = context.trim().split('\n').collect::<Vec<_>>();
...
async fn test_left_anti_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "left_anti", false).await?;
  let context = std::fs::read_to_string(&output_path)?;
  let result = context.trim().split('\n').collect::<Vec<_>>();
...
async fn test_right_semi_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "right_semi", false).await?;
  let context = std::fs::read_to_string(&output_path)?;
  let result = context.trim().split('\n').collect::<Vec<_>>();
...
async fn test_right_anti_join() -> anyhow::Result<()> {
  ...
  lib::cmd::join::run_join(path1, path2, sel1, sel2, "right_anti", false).await?;
  let context = std::fs::read_to_string(&output_path)?;
  let result = context.trim().split('\n').collect::<Vec<_>>();
